...
- [~] 完成动作即时正反馈(建联/解锁/发邮件 → 状态推进 + 列表 +1 + 阶段点亮);真实挣来,不假高潮。R043:反馈层 33 toast 图标去 emoji→◆ + 建联键去 🤝。余:🔴 **confirmUnlock 假反馈**(解锁 toast 称已显示但 UI 未真正 un-mask ██ 数据,违真实挣来红线→专轮修);通知数据 emoji 图标(🤝💬🔔 @765/2037)。
- [ ] 数字可读性:KPI/feed 数字配「对我意味着什么」的解读,去「看不懂的数字」。
- [ ] 各屏空态/死路审计(做完一件事后是否有去向/反馈,有没有空仪表盘)。
