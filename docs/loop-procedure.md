...
- [x] ✅ **T11 删死代码 完成**(R076→R083 分轮):R076 orphan src/data+OnboardingScreen.vue · R080 startScan 死尾+render 空守 · R081 整个 onboarding+canvas 引擎(−455) · R082 死 AI 日报/待办渲染器(−77) · **R083 收尾**(LoginScreen #reg-scan-overlay markup + login.css .rso 块 + onboarding.css 删除 + index.css import + goStep/startAnalysis stub,−156)。全程 build+h1(加 visible 断言)/h3/tour/机检 绿;live app 零回归。残留极小:polish.css 共享选择器里的死 token(.rso-pct/.ob-kpi-val,无害,改 comma-list 有风险故留)。原审计:**R081 删整个死的 onboarding+canvas 引擎**(OB_CHAPTERS_MAP/OB_CONTENTS/runOnboarding/initMapCanvas/drawMap/drawArc/launchArc/advanceOb/showObChapter + MAP_REGIONS/CITY_LIGHTS/CONTINENTS,自包含 0 live 调用,legacy 2298→1843 行 −455,含大块死中文)。**残留 T11 尾**:死 AI_REPORT_ITEMS/TODAY_TODOS(guarded 渲染器,#ai-report-list 已并入 Dashboard)+ LoginScreen `.rso-*` 隐藏 markup + onboarding.css/login.css rso。原始:`#reg-scan-overlay`/`.rso-*` + `#s-onboard`/`runOnboarding`/`OB_CONTENTS` 是死 UI(live=login→网址弹窗→FirstRunAnalysis→enterApp,绕过 scan/onboard,**enterApp 本身 live 必保留**)。**R076 已删**:5 个 orphan `src/data/*.js`(零 import,中文数据副本)+ `OnboardingScreen.vue` 组件 + App.vue import/mount(bundle 97→93.7KB,h1/login/h3/tour✓)。**残留 T11 尾**:`onboarding.css`(`.ob-*` 与 legacy runOnboarding 共用,需先核 .ob-progress-fill/.ob-action-num 是否别处引用再删)+ legacy `runOnboarding`/`OB_CHAPTERS_MAP`/dead startScan 分支 + LoginScreen `.rso-*` 块(改 live 文件,专轮谨慎)。
